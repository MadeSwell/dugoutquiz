...
class Core extends Component {
    ...
    return questions.map((question, index) => {
      ...
      return (
        <div className="result-answer-wrapper" key={index + 1}>
          ...
          <div className="result-answer">{this.renderAnswerInResult(question, userInputIndex)}</div>
  ...
  renderTags(answerSelectionType, numberOfSelection) {
    const {
      appLocale: { singleSelectionTagText, multipleSelectionTagText, pickNumberOfSelection }
    } = this.props;
  ...
  render() {
    ...
    return (
      <div className="questionWrapper">
        {!endQuiz && (
          <div>
            <div className="questionWrapperBody">
              ...
              <div
                style={{
                  opacity: showNextQuestionButton ? 1 : 0.6,
                  pointerEvents: showNextQuestionButton ? "inherit" : "none"
                }}
              >
                <button onClick={() => this.nextQuestion(currentQuestionIndex)} className="nextQuestionBtn btn">
                  {appLocale.nextQuestionBtn}
                </button>
              </div>
            </div>
            <div className="fieldWrapperBody">
              {question.showField && (
                <Field focus={question.field.focus} runners={question.field.runners} outs={question.field.outs} />
              )}
            </div>
          </div>
        )}
        ...
        {endQuiz && !showDefaultResult && customResultPage != undefined && customResultPage(questionSummary)}
      </div>
    );
  }
}
